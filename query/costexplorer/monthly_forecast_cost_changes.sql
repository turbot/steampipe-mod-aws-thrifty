...
select
  ...
  case
    when forecast_month.cost > prev_month.cost then 'alarm'
    else 'ok'
  end as status,
  case
    ...
    when forecast_month.cost < prev_month.cost then prev_month.account_id || ' forecasted monthly cost has decreased by ' || round(cast((prev_month.cost - forecast_month.cost) as numeric), 2) || '.'
    else prev_month.account_id || ' forecasted monthly cost has remained flat.'
  end as reason,
